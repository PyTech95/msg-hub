...
- **Per-tenant WhatsApp Cloud API (Feb 12, 2026)** — each Company Admin can now connect their OWN Meta WhatsApp Business Account. New collection `company_whatsapp_configs` (unique index on `company_id`) storing per-tenant `access_token`/`phone_number_id`/`app_secret`/`graph_version`/`mock`/`is_active` plus an auto-generated unique `verify_token` (`tzs_<24 urlsafe bytes>`). New endpoints: `GET/PUT/DELETE /api/whatsapp/config` (company admin) + `POST /api/whatsapp/config/test` (real Meta Graph health check with tenant creds). Per-tenant webhook: `GET/POST /api/webhook/whatsapp/{company_id}` — tenant-scoped verify token dance + X-Hub-Signature-256 enforcement using tenant's app_secret; inbound messages/statuses tagged with correct `company_id`; unknown company_id → 404. `meta_wa_credentials(company_id)` resolves in order: (1) tenant config, (2) global vault, (3) env fallback. All adapters accept `**kwargs` and Meta adapter uses `company_id` kwarg to pick the right creds. Secret masking: raw `access_token`/`app_secret` never returned by any endpoint — only `*_preview` (last 4 chars) + `*_set` boolean. Company delete cascade removes the WA config row (`company_whatsapp_configs` added to TENANT_COLLECTIONS). New frontend page `/whatsapp-settings` (RoleRoute admin+super_admin; sidebar shows for tenant admins only via new `tenantOnly` flag) with 3-step flow: (1) copy Callback URL + Verify Token to Meta dashboard, (2) save credentials + toggle Mock/Active + Test connection, (3) quick send-test. Super Admin view shows platform overview + list of configured tenants. Removed "Made with Emergent" badge from `public/index.html` per user request. Verified via testing agent v3: 22/22 new backend tests + full frontend UI walkthrough PASS (100%). See `/app/backend/tests/test_iteration9_wa_pertenant.py`.
